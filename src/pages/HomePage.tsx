        ...
        <section id="reviews" className="py-24 bg-background">
          <div className="container mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {[
                { name: "Jake Thompson", trip: "Thurgoona Golf Trip", quote: "Honestly the best weekend we've had in years. Turned up, shirts were waiting, beer was cold, tee times were sorted. I didn't have to organise a single thing." },
                { name: "Mick O'Brien", trip: "Albury Golf Weekend", quote: "As the usual group organiser I'm always stressed before the trip starts. Having BlokesTrips run everything was an absolute game changer. Worth every cent." },
                { name: "Dave Carter", trip: "Murray River Weekend", quote: "Even as a non-golfer I had the best time. Everything was thought of. Already locked in next year's trip and bringing three more of the boys." }
              ].map((t, i) => (
                <div key={i} className="bg-secondary/20 p-10 rounded-[2.5rem] relative hover:scale-[1.02] transition-transform duration-300 shadow-sm hover:shadow-xl hover:bg-secondary/30">
                  ...
                  <p className="text-lg font-medium leading-relaxed mb-8 italic">{t.quote}</p>
                  <div>
                    <h4 className="font-display font-black uppercase italic tracking-tight text-lg leading-none">{t.name}</h4>
                    <p className="text-muted-foreground text-xs uppercase tracking-widest font-bold mt-2">{t.trip}</p>
                  </div>
                </div>
              ))}
            </div>
